...
clear;clc;close all;
disp('Running re_bf_dist_conn_for_he_hs.m - conn vs rebf vs dist')
...
for u = 1:size(rms_match_db_with_sig_bf,1)
    ...
    tone_sig = rms_match_db_with_sig_bf{u,10};
    hc_sig = rms_match_db_with_sig_bf{u,12};
    ...
    all_cases = zeros(5,1);
    for f = 1:5
        t1_rate = mean(tone_rates{f,1}(:, 10:14),2);
        t2_rate = mean(tone_rates{f+2,1}(:, 10:14),2);
        t1t2_rate = mean(hc_rates{f,1}(:, 10:14),2);

        t1_sig = tone_sig(f);
        t2_sig = tone_sig(f+2);
        t1t2_sig = hc_sig(f);

        case_type = indiv_he_hs_classifier(t1_rate, t2_rate, t1t2_rate, t1_sig, t2_sig, t1t2_sig);
        all_cases(f) = case_type;
    end % f

    rms_match_db_with_sig_bf{u,19} = all_cases;
end % u
...
all_roi_info = struct('roi_name', {}, 'rebf', {}, 'x_coord', {}, 'y_coord', {}, 'noise_corr_mat', {}, 'enh_or_sup', {});
...
for k = 1:length(all_roi_keys)
    ...
    roi_enh_or_sup = zeros(n_roi_units, 5);
    ...
    for ru = 1:length(roi_units)
        ...
        % case types
        roi_enh_or_sup(ru,:) = rms_match_db_with_sig_bf{unit_index, 19}';
    end % ru
    ...
    all_roi_info(counter).enh_or_sup = roi_enh_or_sup;
...
all_pairs_rebf_vs_dist = zeros(4, length(dist_bins), length(rebf_bins)); % 4 - HE, HS, NE, NS
...
for i = 1:length(all_roi_info)
    ...
    all_case_types = all_roi_info(i).enh_or_sup;
    ...
    for n1 = 1:n_cells-1
        for n2 = n1+1:n_cells
            ...
            if nc_for_n1_n2 > right_extreme || nc_for_n1_n2 < left_extreme
                ...
                case1_types = all_case_types(n1,:);
                case2_types = all_case_types(n2,:);

                for c = 1:5
                    c1 = case1_types(c);
                    c2 = case2_types(c);

                    all_pairs_rebf_vs_dist(c1, dist_bin, rebf_bin) = all_pairs_rebf_vs_dist(c1, dist_bin, rebf_bin) + 1;    
                    all_pairs_rebf_vs_dist(c2, dist_bin, rebf_bin) = all_pairs_rebf_vs_dist(c2, dist_bin, rebf_bin) + 1;    
                end % c
...
threshold = 5;
all_pairs_rebf_vs_dist(all_pairs_rebf_vs_dist < threshold) = nan;

% %% all_pairs_rebf_vs_dist norm
n_dist_bins_to_see = length(dist_bins);


% % normalise such that sum of each row is 1
type_strs = {'HE', 'HS', 'NE', 'NS'};
for t = 1:4
    each_type_re_bf_vs_dist = squeeze(all_pairs_rebf_vs_dist(t,:,:)); 
    
    % normalise by rebf
    all_pairs_rebf_vs_dist_norm = zeros(n_dist_bins_to_see, length(rebf_bins));
    for i = 1:n_dist_bins_to_see
        all_pairs_rebf_vs_dist_norm(i,:) = each_type_re_bf_vs_dist(i,:)./nansum(each_type_re_bf_vs_dist(i,:));
    end % i
    ...
    figure
        ...
        title([type_strs{t} ' - all pairs rebf vs dist norm: at a dist(d), sum of all 7 rebf = 1' ])
        ylabel('Mid pt of dist(d) bins in micros')
        xlabel([scale ' bins'])
        colorbar()
        xticks(1:length(rebf_bins))
        xticklabels(rebf_bins)
        yticks(1:length(dist_bins))
        yticklabels(dist_bins + dist_bin_size/2)

    % normalise by distance
    all_pairs_rebf_vs_dist_norm = zeros(n_dist_bins_to_see, length(rebf_bins));
    for i = 1:length(rebf_bins)
        all_pairs_rebf_vs_dist_norm(1:n_dist_bins_to_see,i) = each_type_re_bf_vs_dist(1:n_dist_bins_to_see,i)./nansum(each_type_re_bf_vs_dist(1:n_dist_bins_to_see,i));
    end
    ...
    figure
        ...
        title([type_strs{t} '-  all pairs rebf vs dist norm: at a rebf(r), sum of all 20 dist = 1'])
        ylabel('Mid pt of dist(d) bins in microns')
        xlabel([scale ' bins'])
        colorbar()
        xticks(1:length(rebf_bins))
        xticklabels(rebf_bins)
