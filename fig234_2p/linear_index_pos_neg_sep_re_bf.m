% NOT USEFUL and df/f can be negative
% HC - T1 + T2 / HC + T1 + T2
clear; clc; close all

rms_match_file_path = 'E:\RK_E_folder_TTHC_backup\RK TTHC Data\PV\rms_match_db_with_sig_bf.mat';
...
for u = 1:size(rms_match_db,1)
    ...
    for f = 1:5
        t1_rate = mean(mean(all_tone_rates{f,1}(:, 10:14),2));
        t2_rate = mean(mean(all_tone_rates{f+2,1}(:, 10:14),2));
        t1t2_rate = mean(mean(all_hc_rates{f,1}(:, 10:14),2));
