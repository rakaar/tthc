function he_hs_type = he_hs_classifier(tone_rates, hc_rates, tone_sig, hc_sig)
    ...
    for hf=1:5
        ...
        t1_all_iters = mean(tone_rates{t1_i,1}(:,10:14), 2);
        t2_all_iters = mean(tone_rates{t2_i,1}(:,10:14), 2);
        t1t2_all_iters = mean(hc_rates{t1t2_i,1}(:,10:14), 2);
        ...
        t1_sig = sig_convert(tone_sig(t1_i));
        t2_sig = sig_convert(tone_sig(t2_i));
        t1t2_sig = sig_convert(hc_sig(t1t2_i));
...
function new_val = sig_convert(val)
    if val == -1
        new_val = 0;
    else
        new_val = val;
