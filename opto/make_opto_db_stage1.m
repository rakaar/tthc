ephys_data_path = 'Q:\RK_AD_TTHC_OPTO_OLD';
        ...
        if contains(location_dir_name, 'location')
            ...
            units_dir = dir(location_dir_path);
             ...
             for u=3:length(units_dir)
                ...
                if strcmp(unit_data.PP_PARAMS.type, 'AudStimEphys')
                    protochol_type = unit_data.PP_PARAMS.protocol.type;
                    db = unit_data.PP_PARAMS.protocol.stim_protocol.level_hi;
                elseif strcmp(unit_data.PP_PARAMS.type, 'Opto-Ephys')
                    protochol_type = unit_data.PP_PARAMS.protocol.AUD.type;
                    db = unit_data.PP_PARAMS.protocol.AUD.stim_protocol.level_hi;
                end
                ...
                if strcmp(protochol_type, stim_type)
                    for ch=1:length(unit_data.unit_record_spike)
                        ...
                        ephys_db{unit_counter,6} = str2num(unit_filename(regexp(unit_filename, '(\d+)\.')));
                        ephys_db{unit_counter,7} = spikes_all_freqs;
                        ephys_db{unit_counter,8} = unit_data.PP_PARAMS.type;
...
if strcmp(stim_type, 'TonePipSweep')
    opto_tone_db_stage1 = ephys_db;
    save('opto_tone_db_stage1','opto_tone_db_stage1')
elseif strcmp(stim_type, 'HC')
    opto_hc_db_stage1 = ephys_db;
    save('opto_hc_db_stage1', 'opto_hc_db_stage1')
end