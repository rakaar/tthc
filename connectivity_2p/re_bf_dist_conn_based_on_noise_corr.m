% FAILED ATTEMPT AT NOISE CORRELATION ANALYSIS
clc;clear;clc;
...
for k=1:length(tone_keynames)
  ...
  bfs_and_bf0s = zeros(length(units),2);
    for u=1:length(units)
        bfs_and_bf0s(u,1) = rms_match_db_with_sig_bf_and_noise{units(u),11};
        bfs_and_bf0s(u,2) = rms_match_db_with_sig_bf_and_noise{units(u),13};
    end
  ...
  rms_match_noise_corr_db{k,7} = bfs_and_bf0s(:,1);
  rms_match_noise_corr_db{k,8} = bfs_and_bf0s(:,2);
end
...
tone_noise_corr_vec = struct('corr', {}, 'rebf', {}, 'coords1', {}, 'coords2', {});
hc_noise_corr_vec = struct('corr', {}, 'rebf', {}, 'coords1', {}, 'coords2', {});
tone_counter = 1;
hc_counter = 1;
...
for u=1:size(rms_match_noise_corr_db,1)
    ...
    bfs = rms_match_noise_corr_db{u,7};
    % bf0s = rms_match_noise_corr_db{u,8};
    ...
    for r=1:size(nc,1)
        for c=1:size(nc,2)
            if r > c
                ...
                tone_noise_corr_vec(tone_counter).rebf = abs(bfs(r) - bfs(c))*0.25;
    ...
    for r=1:size(nc,1)
        for c=1:size(nc,2)
            if r > c
                ...
                hc_noise_corr_vec(hc_counter).rebf = abs(bfs(r) - bfs(c))*0.25;
