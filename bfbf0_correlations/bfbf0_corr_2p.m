%% PV
clear all;
load('E:\RK TTHC Data\PV\rms_match_db_with_sig_bf.mat')
units_bf = [];
units_bf0 = [];
...
hold off
title(['PV corr coef = ', num2str(bfbf0_corr_mat(1,2)), ' p = ', num2str(p_mat(1,2)), ' R2 = ', num2str(r2)]);
legend('Data Points', 'Fitted Line');

%% SOM

clear all;
load('E:\RK TTHC Data\SOM\rms_match_db_with_sig_bf.mat')
units_bf = [];
units_bf0 = [];
freqs = log2([6,8.5,12,17,24,34,48]);
for u=1:size(rms_match_db_with_sig_bf,1)
...
plot(x, slope*x + intercept);
hold off
title(['SOM corr coef = ', num2str(bfbf0_corr_mat(1,2)), ' p = ', num2str(p_mat(1,2)), ' R2 = ', num2str(r2)]);
legend('Data Points', 'Fitted Line');
...
clear all;
load('E:\RK TTHC Data\Thy\rms_match_db_with_sig_bf.mat')
units_bf = [];
units_bf0 = [];
freqs = log2([6,8.5,12,17,24,34,48]);
for u=1:size(rms_match_db_with_sig_bf,1)
...
plot(x, slope*x + intercept);
hold off
title(['Thy corr coef = ', num2str(bfbf0_corr_mat(1,2)), ' p = ', num2str(p_mat(1,2)), ' R2 = ', num2str(r2)]);
legend('Data Points', 'Fitted Line');
