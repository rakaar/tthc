clear;clc; close all;
rms_match_db = load('E:\RK_E_folder_TTHC_backup\RK TTHC Data\PV\rms_match_db_with_sig_bf.mat').rms_match_db_with_sig_bf;
...
for u = 1:size(rms_match_db,1)
    ...
    if rms_match_db{u,11} ~= -1 && rms_match_db{u,13} == -1
        resp_index = 1;
    elseif rms_match_db{u,11} == -1 && rms_match_db{u,13} ~= -1
        resp_index = 2;
    elseif rms_match_db{u,11} ~= -1 && rms_match_db{u,13} ~= -1
        resp_index = 3;
    end
