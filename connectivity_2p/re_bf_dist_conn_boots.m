...
% bootstrapped data
n_boots = 1000;
chance_all_pairs_rebf_vs_dist_boots = zeros(n_boots, length(dist_bins), length(rebf_bins));
...
for b = 1:n_boots
    ...
    for i = 1:length(all_roi_info)
        ...
        % same indicies to shuffle 
        random_indices = randperm(length(all_bf));
        % shuffle bf
        all_bf = all_bf(random_indices);

        % shuffle positions
        all_x_cord = all_roi_info(i).x_coord;
        all_x_cord = all_x_cord(random_indices);

        all_y_cord = all_roi_info(i).y_coord;
        all_y_cord = all_y_cord(random_indices);

        all_noise_corr = all_roi_info(i).noise_corr_mat;
        % all_noise_corr = all_noise_corr(random_indices, random_indices);
        ...
        for n1 = 1:n_cells-1
            for n2 = n1+1:n_cells
                ...
                if nc_for_n1_n2 > right_extreme || nc_for_n1_n2 < left_extreme
                    ...
                    chance_all_pairs_rebf_vs_dist_boots(b, dist_bin, rebf_bin) = chance_all_pairs_rebf_vs_dist_boots(b, dist_bin, rebf_bin) + 1;
                end 
            end
        end
    end % i
...
% threshold for actual
% threshold = 5;
% for b = 1:n_boots
%     for r = 1:length(dist_bins)
%         for c = 1:length(rebf_bins)
%             if length(actual_all_pairs_rebf_vs_dist_boots(b,r,c)) < threshold
%                 actual_all_pairs_rebf_vs_dist_boots(b,r,c) = nan;
%             end
%         end
%     end
% end % b

% % threshold for chance
% for b = 1:n_boots
%     for r = 1:length(dist_bins)
%         for c = 1:length(rebf_bins)
%             if length(chance_all_pairs_rebf_vs_dist_boots(b,r,c)) < threshold
%                 chance_all_pairs_rebf_vs_dist_boots(b,r,c) = nan;
%             end
%         end
%     end
% end % b
...
% normalise each row in chance by nansum of each row
for b = 1:n_boots
    for r = 1:length(dist_bins)
        chance_all_pairs_rebf_vs_dist_boots(b,r,:) = chance_all_pairs_rebf_vs_dist_boots(b,r,:)/nansum(chance_all_pairs_rebf_vs_dist_boots(b,r,:));
    end
end % b

save(strcat(neuron_type, '_chance.mat'), 'chance_all_pairs_rebf_vs_dist_boots');
