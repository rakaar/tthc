...
for u = 1:size(rms_match_db,1)
    bf = rms_match_db{u,12};
    bf0 = rms_match_db{u,13};
    
    if bf * bf0 < 0  % if atleast one of them is -1
        continue
    end
    ...
    norm_factor_bf = mean(mean(all_tone_rates{bf,1}(:,501:570),2));
    norm_factor_bf0 = mean(mean(all_hc_rates{bf0,1}(:,501:570),2));

    

    for f = 1:5
        t1_spike_rates = mean(all_tone_rates{f,1}(:, 501:570),2)./norm_factor_bf;
        t2_spike_rates = mean(all_tone_rates{f+2,1}(:,501:570),2)./norm_factor_bf;
        t1t2_spike_rates = mean(all_hc_rates{f,1}(:, 501:570), 2)./norm_factor_bf0;
