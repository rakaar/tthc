...
for u = 1:size(rms_match_db,1)
    ...
    all_cases = zeros(5,1);
    for f = 1:5
        t1_rate = mean(tone_rates{f,1}(:, 501:570),2);
        t2_rate = mean(tone_rates{f+2,1}(:, 501:570),2);
        t1t2_rate = mean(hc_rates{f,1}(:, 501:570),2);
...
% make roi db
all_roi_info = struct('roi_name', {}, 'channels', {}, 'bf', {}, 'noise_corr', {}, 'enh_or_sup', {});
...
for k = 1:length(roi_keynames)
    ...
    enh_or_sup_vec = zeros(length(units),5);
    if length(units) > 1
        for u = 1:length(units)
            ...
            enh_or_sup_vec(u,:) = rms_match_db{unit,15}';
        end % u
        ...
        all_roi_info(counter).enh_or_sup = enh_or_sup_vec;
        counter = counter + 1;
    end % if
end % k
...
all_pairs_rebf_vs_dist = zeros(4, length(dist), length(rebf_bins)); % 4 - HE, HS, NE, NS
    ...
    for c1 = 1:n_channels-1
        for c2 = c1+1:n_channels
            ...
            if nc_c1_c2 > right_extreme || nc_c1_c2 < left_extreme
                ...
                case1_types = all_case_types(c1,:);
                case2_types = all_case_types(c2,:);

                for c = 1:5
                    case1 = case1_types(c);
                    case2 = case2_types(c);

                    all_pairs_rebf_vs_dist(case1, dist_index, rebf_index) = all_pairs_rebf_vs_dist(case1, dist_index, rebf_index) + 1;    
                    all_pairs_rebf_vs_dist(case2, dist_index, rebf_index) = all_pairs_rebf_vs_dist(case2, dist_index, rebf_index) + 1;    
                end % c
...
n_dist_bins_to_see = length(dist);
