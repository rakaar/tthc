...
figpath = '/media/rka/Elements/RK_E_folder_TTHC_backup/RK TTHC figs eps/figThy/examples_aug32';
combiner = '***';
...
%         for k = 1:7
%             tone_mean = mean(tone_res{k,1}(:, 5:19),2);
%             hc_mean = mean(hc_res{k,1}(:, 5:19),2);

%             % sig btn tone spont and hc spont
%             if mean(tone_mean) > mean(tone_spont) 
%                 tone_sig(k) = ttest2(tone_mean, tone_spont);
%             end

%             if mean(hc_mean) > mean(hc_spont) 
%                 hc_sig(k) = ttest2(hc_mean, hc_spont);
%             end

%             tone_tuning(k) = mean(tone_mean);
%             hc_tuning(k) = mean(hc_mean);

%             tone_err(k) = std(tone_mean)/sqrt(size(tone_mean,1));
%             hc_err(k) = std(hc_mean)/sqrt(size(hc_mean,1));
...
%         hold off
%         title([loc ' ++++ ' num2str(u) ' Tone Sig ' num2str(tone_sig') ' HC Sig ' num2str(hc_sig') ])
%         pause
%         clf;
%     end
...
% loc_units = loc_units_map(loc);
loc_units = [14637, 14640, 14709, 14720];
for j = 1:length(loc_units)
    u = loc_units(j);
    tone_res = rms_match_db{u,8};
    hc_res = rms_match_db{u,9};
   ...
   % ---------------- individual traces
        figure
            plot(gaussmoth(tone_ex_dff, 0.7))
...
% loc_units = [  15390, 15362 ];
    ...
    for j = 1:length(loc_units)
        ...
        for k = 1:5
            ...
            if tone_sig_1 + tone_sig_2 + hc_sig_1 ~= 3 || case_type ~= 3
                continue;
             end
            % tuning
